...
A single shared password gates the whole app — no accounts, this is a personal single-user tool. A FastAPI middleware (not per-route `Depends`, so a new route can't accidentally ship unprotected) requires a valid signed session cookie on every `/chat`, `/models`, `/conversations`, `/tasks`, `/habits`, and `/auth` request; `/login`, `/logout`, `/health`, and the static SPA shell stay public — the shell is just JS/CSS and can't fetch anything real without logging in first. The frontend checks `/auth/check` on load and shows a login screen if it 401s; any other API call that gets a 401 (session expired mid-use) fires a `hermes:unauthorized` window event that does the same, so a stale session doesn't just leave views silently failing.
...
- No HTTPS — login password and session cookie travel as plaintext HTTP on the LAN. Acceptable for a trusted home network for now, but worth revisiting if that assumption ever changes
- No remote/outside-the-LAN access (e.g. Tailscale) — everything above is LAN-only by design
