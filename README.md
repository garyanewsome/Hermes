...
- Postgres (`psycopg`) for tasks/habits/todo lists/notes — a separate `planner` database, not Hindsight's. Hindsight is semantic recall over durable facts; this is mutable structured state that doesn't belong in a recall-by-similarity store.
...
```
Opens on `http://localhost:5173` and proxies `/chat`, `/models`, `/conversations`, `/tasks`, `/habits`, `/todo-lists`, `/todo-items`, `/notes`, `/login`, `/logout`, `/auth`, `/health` to the backend on port 8001 (see `vite.config.js`) — run both at once for a working dev setup.
